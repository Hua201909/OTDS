...
from losses.baseline import baseline_loss
...
# Number model (BiLSTM parameters)
model = BiLSTMModel(
    input_size=5,
    lstm_hidden_size=32,
    lstm_layers=1,
...
def loss_func(model, x, y):
    return baseline_loss(model, x, y)
...
results = run_experiment(
    ...
    epochs=EPOCHS,
    patience=PATIENCE,
    model_save_path=os.path.join(MODELS_DIR, "bilstm_baseline.pth")
)

import json
with open(os.path.join(RESULTS_DIR, "bilstm_baseline.json"), "w") as f:
